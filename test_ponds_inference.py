...
output_folder = os.path.join(os.environ['DATA'],
                             'ponds_and_wells',
                             'output',
                             str(zoom_level),
                             block_name)

csv_file = os.path.join(output_folder,
                        block_name + '.csv')
...
if not os.path.exists(data_download_folder):
    ...
    download_map_tiles(base_url, data_download_folder, zoom_level,
                   scale, topLeft, topRight, bottomLeft, bottomRight)
else:
...
def get_entropy(img, mask):
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY).astype(
        np.float32)  # Convert to grayscale
    ...
    ent = skimage.filters.rank.entropy(img_gray.copy(),
        ...
        for i, (polygon, cls, conf) in enumerate(zip(results[0].masks.xy, results[0].boxes.cls.cpu().numpy(), results[0].boxes.conf.cpu().numpy())):
            ...
            if conf >= conf_thresholds[class_name]:
                ...
                if class_name == 'Wet':  # Only compute entropy for Wet class
                    predicted_mask = results[0].masks.data.cpu(
                    ).numpy()[i].astype(np.uint8)  # Convert to uint8
...
def extract_xtile_ytile(image_path):
    try:
        ...
        if len(parts) >= 4:
            ...
            raise ValueError(
                "Filename does not contain valid tile coordinates")
    except Exception as e:
        raise ValueError(
...
for current_image in image_files:
    ...
    if polygons:
        max_vertices = max(max_vertices, max(len(polygon)
...
for _, row in df.iterrows():
    ...
    while True:
        ...
        if pd.notna(x) and pd.notna(y):
            lon, lat = pixel_to_geo(x, y, lat_top_left, lon_top_left,
                                    lat_bottom_right, lon_bottom_right, tile_width, tile_height)
...
zip_filename = os.path.join(output_folder,
                            f"{csv_basename}_COMBINED_GEOMETRY.zip")
shapefile_files = glob.glob(os.path.join(
    output_folder, f"{csv_basename}_COMBINED_GEOMETRY.*"))

# Exclude the .csv file from the list of files to be zipped
shapefile_files = [
    file for file in shapefile_files if not file.endswith('.csv')]
